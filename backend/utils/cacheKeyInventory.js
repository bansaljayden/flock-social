...
const INVENTORY = [
  ...
  },
  {
    ...
    protects: 'no spend; correctness. It holds the SOS pushes to one person from one sender that are still on their way, which kind landed last, and what was sent, so that once the last of them answers, what landed last is checked against emergency_alerts and the right alarm or all-clear is sent again (rule 4 of AN SOS ALARM MUST NOT OUTLIVE ITS ALL-CLEAR, NOR AN ALL-CLEAR A NEWER ALARM)',
    denominator: 'SOS pushes still waiting on the provider, per person and sender',
    bound: 'self-draining: a push leaves its slot when its send answers, and a slot is deleted once its last push has answered and what landed last agrees with Postgres; hard ceiling SOS_SLOTS_MAX 5000 slots, past which a push is sent unregistered; per slot, the alarms of the SOS_SLOT_ALARMS_KEPT (4) newest alerts and one all-clear are kept, and at most SOS_SLOT_RESENDS_MAX (3) resends are made',
    verdict: 'SAFE',
    why: 'Only an SOS or its stand-down can open a slot, and /alert holds each account to one send per sixty second floor and six attempts per fifteen minutes, each fanned out to the flockmates on a confirmed plan within twelve hours; the stand-down is metered as well, three that reach somebody and twelve attempts per fifteen minutes. A push stays in its slot as long as one provider send, which always settles: on firebase-admin\'s own retries and timeouts, or, past the 8 second deadline, through `settled`, which never rejects. The slot ceiling covers sends that stop answering altogether and gives up only the resend, never a push; the resend cap covers a truth that keeps moving, so a slot cannot send without end. One instance is enough because this app runs one (numReplicas 1 on Railway): every SOS push starts in this process. At two instances a push running on the other one would not be in this slot; rules 1 and 3, which read Postgres before each send, would still hold there, and only the resend would be lost.',
  },
