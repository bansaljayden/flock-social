...
const INVENTORY = [
  ...
  },
  {
    ...
    bound: 'self-draining: a push leaves its slot when its send answers, and a slot is deleted once its last push has answered and every device it reached agrees with Postgres or is still registered and waiting on the retry its own failed correction queued; hard ceiling SOS_SLOTS_MAX 5000 slots, past which a push is sent unregistered; per slot, one entry per device (a person has at most 20 device rows, MAX_TOKENS_PER_USER), the ids the slot\'s pushes were addressed to (at most 20 per push), and one all-clear; the corrections it sends are counted in sosCorrections, and none is sent past the alert\'s deadline, six hours from its created_at (rule 5); at most three re-checks (SOS_RECHECK_DELAYS_MS) when Postgres cannot be read',
    verdict: 'SAFE',
    why: 'Only an SOS or its stand-down can open a slot, and /alert holds each account to one send per sixty second floor and six attempts per fifteen minutes, each fanned out to the flockmates on a confirmed plan within twelve hours; the stand-down is metered as well, three that reach somebody and twelve attempts per fifteen minutes. A push stays in its slot as long as one provider send, which always settles: on firebase-admin\'s own retries and timeouts, or, past the 8 second deadline, through `settled`, which never rejects. A slot that could not read Postgres waits at most three times (five seconds, thirty, two minutes) before it is let go. The slot ceiling covers sends that stop answering altogether and gives up only the correction, never a push; the per-device count in sosCorrections covers a truth that keeps moving and a device the provider keeps refusing inside the alert\'s window, and rule 5 ends every chain at the alert\'s deadline, which is read from Postgres and which nothing in memory can reset, so neither a slot nor a chain of them can send without end. One instance is enough because this app runs one (numReplicas 1 on Railway): every SOS push starts in this process. At two instances a push running on the other one would not be in this slot; rules 1 and 3, which read Postgres before each send, would still hold there, and only the correction would be lost.',
  },
  {
