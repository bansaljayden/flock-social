...
const INVENTORY = [
  ...
  },
  {
    ...
    bound: 'at most SOS_CORRECTIONS_PER_KEY (3) corrections per key; SOS_CORRECTIONS_MAX 5000 keys, the least recently corrected out first (delete-before-set); an entry older than SOS_CORRECTION_TTL_MS (six hours, CANCEL_WINDOW_MS in routes/safety.js) is not used and is deleted when read',
    verdict: 'SAFE',
    why: 'Only a correction adds to it, and corrections follow SOS pushes, which /alert and the stand-down meter per account, so the key space is bounded by real alerts, their flockmates\' devices (at most 20 each) and one account key per target. An entry past six hours has nothing left to correct: the stand-down covers alerts from the last six hours, and a queued retry lives thirty minutes. Evicting an entry early can only give a device a few more corrections, each still a single bounded send. It lives in this process like the slot it serves; a deploy empties it, and a correction after that starts its count again.',
  },
  {
