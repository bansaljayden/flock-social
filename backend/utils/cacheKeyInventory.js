...
const INVENTORY = [
  ...
  // ── services/moneyHub.js ──────────────────────────────────────────────────
  {
    ...
    key: "the source and the inputs its read depends on: 'stripe:<first day of the month>', 'revenuecat:<first day of the month>:<count>:<sha256 prefix of the Pro account ids asked about>', 'besttime:key' (the key endpoint takes no input but the key, which is never in the cache key) and 'model:<window days>d:<the band ladder cuts, read off crowdEngine.getLabel>'",
    callerControls: 'nothing: the month comes from the server clock, the account ids from users.is_premium, the model window is a constant and the ladder is crowdEngine\'s. GET /api/admin/money is admin only (requireAdmin on the whole router), and its one input, ?refresh=1, is honoured only once the held answer is a minute old',
    protects: 'Stripe, RevenueCat and BestTime read quotas, and a Postgres join: a cold hub read is about ten Stripe list calls, up to two hundred RevenueCat subscriber reads, one BestTime key-endpoint read (it spends no credit, and it is still not ours to hammer), and one join of a month of served_predictions against the live ml_training_data readings',
    denominator: 'n/a, not a counter: one held answer per question, 5 minutes after a good vendor read and 1 minute after a failed one; the served-forecast check is held for an hour after a good answer, because the readings it scores against land hourly; a read in flight is shared only by requests asking the same question',
    bound: 'one settled answer per source: storing an answer deletes every other settled key of that source, so at rest the map holds at most four keys, one per source, plus a read in flight for a question asked mid-read',
    verdict: 'SAFE',
    why: 'The inputs are in the key because the reads depend on them: under a fixed key, the first read after midnight on the 1st served last month\'s balance as this month\'s, and a RevenueCat tally taken before somebody subscribed kept not counting them until it expired. The model check keys on its window and on the ladder it scores with, so a re-cut ladder is a new question rather than an old answer. No input comes from the request, so a caller cannot grow the map, and an answer to a question nobody can ask again is dropped the moment a newer one lands. It lives in this process: a deploy empties it, and a second instance would hold its own copy and read the vendors, and run the check, on its own schedule. This app runs on one instance (numReplicas 1 on Railway), and the cost of a second would be one extra read per source per hold, not a hole.',
  },
