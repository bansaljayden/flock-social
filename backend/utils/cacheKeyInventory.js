...
const INVENTORY = [
  ...
  },
  {
    ...
    protects: 'no spend; correctness. It holds the SOS pushes to one person from one sender that are still on their way, which copy each of that person\'s devices accepted last and in what order, the newest all-clear sent to them, and which corrections have gone to which device, so that once the last push answers, each device is checked against emergency_alerts and a device on the wrong push is sent the right one, alone (rule 4 of AN SOS ALARM MUST NOT OUTLIVE ITS ALL-CLEAR, NOR AN ALL-CLEAR A NEWER ALARM)',
    denominator: 'SOS pushes still waiting on the provider, per person and sender',
    bound: 'self-draining: a push leaves its slot when its send answers, and a slot is deleted once its last push has answered and every device it reached agrees with Postgres; hard ceiling SOS_SLOTS_MAX 5000 slots, past which a push is sent unregistered; per slot, one entry per device (a person has at most 20 device rows, MAX_TOKENS_PER_USER), one all-clear, at most SOS_SLOT_RESENDS_MAX (3) corrections with each correction sent to a device once, and at most three re-checks (SOS_RECHECK_DELAYS_MS) when Postgres cannot be read',
    verdict: 'SAFE',
  ...
  },
  {
    ...
    protects: 'no spend; accuracy. It keeps the exact alarm each alert went out with, the radius of the fix included, which Postgres does not store, so rule 4 sends a device the same alarm again rather than one rebuilt without the radius',
