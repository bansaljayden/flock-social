...
const INVENTORY = [
  ...
  // ── services/moneyHub.js ──────────────────────────────────────────────────
  {
    ...
    key: "the source and the inputs its read depends on: 'stripe:<first day of the month>', 'revenuecat:<first day of the month>:<count>:<sha256 prefix of the Pro account ids asked about>', 'besttime:key' (the key endpoint takes no input but the key, which is never in the cache key), 'model:<window days>d:<the band ladder cuts, read off crowdEngine.getLabel>' and 'database:roundtrip' (the one SELECT 1 the operator's database step times; it takes no input, and the host it reaches is never in the key)",
    callerControls: 'nothing: the month comes from the server clock, the account ids from users.is_premium, the model window is a constant, the ladder is crowdEngine\'s and the round-trip key is a constant. GET /api/admin/money is admin only (requireAdmin on the whole router), and its one input, ?refresh=1, is honoured only once the held answer is a minute old',
    protects: 'Stripe, RevenueCat and BestTime read quotas, and Postgres: a cold hub read is about ten Stripe list calls, up to two hundred RevenueCat subscriber reads, one BestTime key-endpoint read (it spends no credit, and it is still not ours to hammer), one join of a month of served_predictions against the live ml_training_data readings, and one SELECT 1 on a pooled connection',
    denominator: 'n/a, not a counter: one held answer per question, 5 minutes after a good vendor read and 1 minute after a failed one, and the database round trip under the same two holds; the served-forecast check is held for an hour after a good answer, because the readings it scores against land hourly; a read in flight is shared only by requests asking the same question',
    bound: 'one settled answer per source: storing an answer deletes every other settled key of that source, so at rest the map holds at most five keys, one per source, plus a read in flight for a question asked mid-read',
    verdict: 'SAFE',
    why: 'The inputs are in the key because the reads depend on them: under a fixed key, the first read after midnight on the 1st served last month\'s balance as this month\'s, and a RevenueCat tally taken before somebody subscribed kept not counting them until it expired. The model check keys on its window and on the ladder it scores with, so a re-cut ladder is a new question rather than an old answer. The round trip has a constant key because it has no input: it times the pool this process already holds, so there is one question and one answer to it. No input comes from the request, so a caller cannot grow the map, and an answer to a question nobody can ask again is dropped the moment a newer one lands. It lives in this process: a deploy empties it, and a second instance would hold its own copy and read the vendors, and run the check, on its own schedule. This app runs on one instance (numReplicas 1 on Railway), and the cost of a second would be one extra read per source per hold, not a hole.',
  },
  ...
  },
  {
    ...
    verdict: 'SAFE',
    why: 'Added from the code review of 2026-09-05: the share was tracked per '
  ...
  {
    file: 'services/pushHelper.js', name: 'sosSlots', kind: 'inflight',
    key: '`${recipientId}|${senderId}` of an SOS alarm or all-clear push: the sender is the account that pressed SOS or stood it down, the recipient one of the flockmates routes/safety.js read from the database for it',
    callerControls: 'nothing in a request: the sender is the authenticated account, and the recipients come from SOS_FLOCK_AUDIENCE_SQL or an alert\'s recorded audience, never from the body',
    protects: 'no spend; correctness. It holds the SOS pushes to one person from one sender that are still on their way, which copy each of that person\'s devices accepted last and in what order (or that a send to it failed without saying the copy never arrived), with the stand-down an all-clear carried and whether the push was a correction and to what, the devices a push that went to the provider was addressed to, and the newest all-clear sent to them, so that once the last push answers, each device is checked against emergency_alerts and a device on the wrong push, or unsure of what it holds, is sent the right one, alone, and one whose row has gone is stood in for by the devices registered now (rule 4 of AN SOS ALARM MUST NOT OUTLIVE ITS ALL-CLEAR, NOR AN ALL-CLEAR A NEWER ALARM)',
    denominator: 'SOS pushes still waiting on the provider, per person and sender',
    bound: 'self-draining: a push leaves its slot once its send has answered and the retry and ledger row that answer calls for are written, and a slot is deleted once its last push has answered and every device it reached agrees with Postgres or is still registered and waiting on the retry its own failed correction queued; hard ceiling SOS_SLOTS_MAX 5000 slots, past which a push is sent unregistered; per slot, one entry per device (a person has at most 20 device rows, MAX_TOKENS_PER_USER), the ids the slot\'s pushes were addressed to (at most 20 per push), and one all-clear; the corrections it sends are counted in sosCorrections, and none is started past the alert\'s deadline, six hours from its created_at (rule 5); at most three re-checks (SOS_RECHECK_DELAYS_MS) when Postgres cannot be read',
    verdict: 'SAFE',
    why: 'Only an SOS or its stand-down can open a slot, and /alert holds each account to one send per sixty second floor and six attempts per fifteen minutes, each fanned out to the flockmates on a confirmed plan within twelve hours; the stand-down is metered as well, three that reach somebody and twelve attempts per fifteen minutes. A push stays in its slot as long as one provider send and the bookkeeping that follows its answer (its retry and its ledger row, a few statements whose failures are caught), and the send always settles: on firebase-admin\'s own retries and timeouts, or, past the 8 second deadline, through `settled`, which never rejects. A slot that could not read Postgres waits at most three times (five seconds, thirty, two minutes) before it is let go. The slot ceiling covers sends that stop answering altogether and gives up only the correction, never a push; the per-device count in sosCorrections covers a truth that keeps moving and a device the provider keeps refusing inside the alert\'s window, and rule 5 starts no link of any chain after the alert\'s deadline, which is read from Postgres and which nothing in memory can reset, so neither a slot nor a chain of them can send without end. One instance is enough because this app runs one (numReplicas 1 on Railway): every SOS push starts in this process. At two instances a push running on the other one would not be in this slot; rules 1 and 3, which read Postgres before each send, would still hold there, and only the correction would be lost.',
  },
  {
    file: 'services/pushHelper.js', name: 'sosCorrections', kind: 'counter',
    key: '`${deviceId}|${target}` for a correction to a known device, or `u${recipientId}|${target}` for devices sent to in place of one whose row has gone, where target is rule 4\'s `alarm|<alertId>` or `clear|<alertId>`: device ids and alert ids are rows the server read, and the recipient is a flockmate routes/safety.js read from the database',
    callerControls: 'nothing in a request: every part of the key is a row the server read from Postgres',
    protects: 'no spend; a bound. It counts the corrections rule 4 has sent one device for one push, so a correction that fails, is queued like any failed push, and fails again when that retry is released (in a new slot each time) cannot be sent without end; per device, so no device\'s history stops another device being corrected',
    denominator: 'devices corrected to one SOS push inside the stand-down window',
    bound: 'at most SOS_CORRECTIONS_PER_KEY (3) corrections per key; SOS_CORRECTIONS_MAX 5000 keys, the least recently corrected out first (delete-before-set); an entry older than SOS_ALERT_WINDOW_MS (six hours, CANCEL_WINDOW_MS in routes/safety.js) is not used and is deleted when read. The count is the fine bound, not the last one: whatever it holds, no correction is started, and nothing is queued or released, past the alert\'s deadline (rule 5)',
    verdict: 'SAFE',
    why: 'Only a correction adds to it, and corrections follow SOS pushes, which /alert and the stand-down meter per account, so the key space is bounded by real alerts, their flockmates\' devices (at most 20 each) and one account key per target. An entry past six hours has nothing left to correct: rule 5 starts no correction, and queues or releases no SOS push, past six hours from the alert\'s created_at. An eviction is not harmless on its own: the key\'s count starts again, and more than 5000 keys corrected inside one window could push one another out for as long as their corrections kept failing, so this count alone does not end a chain. The alert\'s deadline does. It is read from Postgres on every check and every queued row, no eviction moves it, and an eviction can add corrections only inside that window. It lives in this process like the slot it serves; a deploy empties it, and a correction after that starts its count again, inside the same deadline.',
  },
  {
    file: 'services/pushHelper.js', name: 'sosAlarmCopies', kind: 'cache',
    key: 'the id of an emergency_alerts row whose alarm this process sent, taken from the push routes/safety.js built for it',
    callerControls: 'nothing: the id is the one /alert claimed for the authenticated account, carried on the push by the server and never read from a request',
    protects: 'no spend; accuracy. It keeps the exact alarm each alert went out with, the radius of the fix included, which Postgres does not store, so rule 4 sends a device the same alarm again rather than one rebuilt without the radius (which is worded as an approximate location)',
    denominator: 'alerts whose alarm was sent in the last thirty minutes',
    bound: 'SOS_ALARM_COPIES_MAX 1000 entries, oldest out first (delete-before-set moves a re-sent alert to the back); an entry older than SOS_ALARM_COPY_TTL_MS (30 minutes) is never used and is deleted when read',
    verdict: 'SAFE',
    why: 'Only a sent SOS alarm adds an entry, and /alert meters those per account (the sixty second floor, six attempts per fifteen minutes), so the key space is bounded by real alerts and cannot be grown by a caller. A miss costs one read of emergency_alerts and users, and the alarm built from it loses only the fix radius. The entry is not needed past thirty minutes, the life of an outbox retry. It lives in this process like the slot it serves, and a deploy empties it, which is the case the Postgres rebuild exists for.',
  },
