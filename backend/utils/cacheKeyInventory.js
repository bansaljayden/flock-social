...
const INVENTORY = [
  ...
  },
  {
    ...
    protects: 'no spend; correctness. It holds the SOS pushes to one person from one sender that are still on their way, which copy each of that person\'s devices accepted last and in what order (or that a send to it failed without saying the copy never arrived), the newest all-clear sent to them, and which corrections have gone to which device, so that once the last push answers, each device is checked against emergency_alerts and a device on the wrong push, or unsure of what it holds, is sent the right one, alone (rule 4 of AN SOS ALARM MUST NOT OUTLIVE ITS ALL-CLEAR, NOR AN ALL-CLEAR A NEWER ALARM)',
