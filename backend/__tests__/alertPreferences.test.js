...
//      joined on ' — '. DESIGN-STANDARD rule 1 forbids it in anything a user reads,
...
for (const rel of ['routes/flocks.js', 'routes/venueProfile.js', 'services/crowdAlerts.js']) {
  test(`no em dash reaches a user from ${rel}`, () => {
    ...
    assert.deepStrictEqual(
      ...
      'DESIGN-STANDARD rule 1: no em dash in user-visible text. Use a period, a comma, or restructure.'
    );
  });
}
...
// feature that was never built, which DESIGN-STANDARD rule 5 forbids. Wiring it here
