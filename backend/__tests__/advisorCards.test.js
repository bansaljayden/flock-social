...
// ── 6. DESIGN-STANDARD and the no-write pin ───────────────────────────────────────
...
test('no owner-visible string carries an em dash or class words, and the guard itself bites', async () => {
  ...
  assert.throws(() => advisorFacts.assertCleanCopy('busy — tonight', 'test'), /DESIGN-STANDARD/);
  assert.throws(() => advisorFacts.assertCleanCopy('a seamless evening', 'test'), /DESIGN-STANDARD/);
});
