...
test('a run that will not end is ended, so the next hour can start', () => {
  ...
  // run open.
  assert.match(collect, /const MAX_RUN_MS = 3 \* 60 \* 60 \* 1000;/);
  ...
  // Far past any legitimate run: fifty minutes of calls plus a corpus-wide
  // baseline refresh that can take a good part of an hour. A watchdog that
  // could fire inside the refresh would roll it back every hour; one that
  // fires at three hours catches only a run that has stopped doing anything.
  const m = collect.match(/const MAX_RUN_MS = (\d+) \* 60 \* 60 \* 1000;/);
  assert.ok(m && Number(m[1]) >= 2 && Number(m[1]) <= 6, 'the watchdog must sit hours past a legitimate run, not minutes');
});
