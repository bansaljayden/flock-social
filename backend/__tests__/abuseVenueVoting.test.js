...
// What is still open:
//
//   U. THE TALLY HAS NO CLOCK. POST/DELETE /api/flocks/:id/vote check
//      membership and nothing else. A flock that is confirmed, completed or
//      cancelled still accepts votes and still recomputes and broadcasts the
//      standings, so "the venue was already picked" is not a state the vote
//      routes know about.
//
//   V. GUESTS OUTVOTE MEMBERS. A guest_votes row counts one-for-one with a
//      member vote in vote_count, and the guest cap is 50 per flock against a
//      member roster the invite budget keeps far smaller. Anyone holding the
//      share link can create guest RSVPs up to that cap and vote all of them,
//      with no account, no email and no membership.
//
...
test('ABUSE U: a completed flock still accepts votes and still moves the standings', async () => {
  as(1, 'Ava'); as(2, 'Bo');
  ...
  const standings = r.body.votes.map((v) => [v.venue_name, v.vote_count]).sort();
  assert.deepStrictEqual(standings, [['Somewhere Else', 1], ['The Chosen Bar', 1]],
    'the evening is over and the tie is live again');

  // Nothing on the vote path ever reads flocks.status.
  assert.strictEqual(log.some((q) => /FROM flocks/.test(q.sql)), false,
    'the vote routes never load the flock row at all, so no state of it can refuse a vote');
  assertQueriesUnderstood();
});

test('ABUSE U2: a vote can also be withdrawn after the fact, changing what the history says the group chose', async () => {
  ...
  as(2, 'Bo');
  await call('DELETE', `/api/flocks/${FLOCK}/vote`);

  as(3, 'Cy');
  const after = await votes();
  assert.deepStrictEqual(after.body.votes.map((v) => v.venue_name), ['Ramen'],
    'the venue that lost is now the only one that was ever voted for');
  assertQueriesUnderstood();
});
...
test('ABUSE V: guest votes count one-for-one with members, up to fifty per flock', async () => {
  as(1, 'Ava'); as(2, 'Bo'); as(3, 'Cy');
  world.members.push({ user_id: 1, status: 'accepted' }, { user_id: 2, status: 'accepted' }, { user_id: 3, status: 'accepted' });
  ...
  // routes/guest.js refuses at 50 RSVPs per flock, per its own transaction.
  ...
  assert.strictEqual(rows[0].venue_name, 'The Link Holders Pick');
  assert.strictEqual(rows[0].vote_count, 50);
  assert.strictEqual(rows[0].guest_count, 50);
  assert.strictEqual(rows[0].voters.length, 0,
    'and nobody is named for any of them, so there is nothing on the screen to argue with');
