...
router.get('/search',
  ...
  async (req, res) => {
    try {
      ...
      const radiusMiles = parseInt(req.query.radius) || 30;
...
// GET /api/events/featured?location=lat,lng — curated "what's happening nearby" for discover page
router.get('/featured',
  [
    query('location').trim().isLength({ min: 3 }).withMessage('Location (lat,lng) is required'),
  ],
  async (req, res) => {
    try {
      ...
      const location = req.query.location;
      const cacheKey = `featured:${location}`;
      const cached = getCached(cacheKey);
      if (cached) return res.json(cached);
