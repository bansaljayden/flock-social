...
// DESIGN-STANDARD, applied to every owner-visible string this module emits: no em
...
function assertCleanCopy(text, where) {
  ...
  for (const banned of BANNED_COPY) {
    if (lower.includes(banned)) {
      throw new Error(`advisor copy violates DESIGN-STANDARD (${where}): contains "${banned === '—' ? 'em dash' : banned}"`);
    }
  }
}
...
function externalText(value, { max = EXTERNAL_TEXT_MAX } = {}) {
  ...
  for (const ch of String(value)) {
    ...
    if (DASH_CODEPOINTS.has(cp)) { s += '-'; continue; }  // design rule 1, no throw
