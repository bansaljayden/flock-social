...
export function validateListing(listing) {
  ...
  // House rule (DESIGN-STANDARD): no em dashes in any user-visible text.
  for (const [field, value] of Object.entries({ ...fields, copyright: listing.copyright })) {
    if (value && value.includes(EM_DASH)) {
      err(field, `${field} contains an em dash, which the copy standard bans. FIX: replace it with a period, comma, or restructure (DESIGN-STANDARD.md rule 1).`);
    }
  }
