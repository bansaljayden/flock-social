...
/* checkPermissions, with a clock. Resolves to one of:
   ...
   ONE PROBE AT A TIME, AND ONLY THE NEWEST ONE MAY SPEAK. Two requests in the
   same moment (Discover and the toggle both fire on the tab) share one
   question rather than asking it twice, and a probe that has been superseded
   cannot set or clear `bridgeSilent` when it finally finishes: a hung probe
   from a minute ago timing out after a newer one heard the plugin must not
   send the next request to WebKit, and an old late answer must not undo a
   newer silence. The sequence number is the whole of that rule. */
...
function probe() {
  ...
  const mine = ++probeSeq;
  const current = () => mine === probeSeq;
  probeFlight = new Promise((resolve) => {
    let done = false;
    const answer = (result) => {
      if (done) return;
      done = true;
    ...
    load().then((Geolocation) => {
      ...
      // A late answer counts too: the plugin spoke, so the bridge is alive.
      const heard = () => { clearTimeout(timer); if (current()) bridgeSilent = false; };
      try {
  ...
  const release = () => { if (probeFlight === flight) probeFlight = null; };
  const flight = probeFlight;
  flight.then(release, release);
  return flight;
}

// A finite, non-negative `timeout` is a clock, zero included: the web API
// reads zero as "a cached fix now, or fail", and a caller who wrote 0 did not
// mean forever. Anything else is no clock at all.
...
const webCode = (err) => (
  err && [PERMISSION_DENIED, POSITION_UNAVAILABLE, TIMEOUT].includes(err.code)
    ? err.code
...
/* navigator.geolocation on a device: WebKit's API, which does not use the
   ...
   First answer wins here as well, under a clock of this module's own. The
   SOS path in App.js already learned that a WebView can ignore the `timeout`
   it was handed, and a fallback that can hang is not a fallback. */
function webOnDevice(onSuccess, onError, options, reason) {
  ...
  const succeed = once((position) => { if (typeof onSuccess === 'function') onSuccess(position); });
  const failWeb = once((code, message) => fail(onError, code, message, 'webkit/' + reason));
  const ms = clockFor(options && options.timeout);
  if (ms !== null) timer = setTimeout(() => failWeb(TIMEOUT, 'Timed out getting your location.'), ms);
  try {
    geo.getCurrentPosition(
      succeed,
      (err) => failWeb(webCode(err), (err && err.message) || 'Could not get a location.'),
      options,
    );
  } catch (err) {
    failWeb(POSITION_UNAVAILABLE, (err && err.message) || 'Could not get a location.');
  }
}
...
export function watchPosition(onSuccess, onError, options) {
  ...
  // Same fallback as getCurrentPosition: a bridge that does not answer, or a
  // chunk that did not load, is not a reason to leave a person without the
  // location WebKit can give. The web id rides on the handle so clearWatch
  // can stop it.
  const watchViaWeb = () => {
  ...
  probe().then((answer) => {
    if (handle.cancelled) return;
    if (answer.state === 'silent' || answer.state === 'no-plugin') { watchViaWeb(); return; }
    if (answer.state === 'denied') {
      fail(onError, PERMISSION_DENIED, 'Location permission was denied.', 'probe-denied');
      return;
    }
    if (answer.state === 'error') { failFromPlugin(onError, answer.pluginErr); return; }
    load().then((Geolocation) => {
      if (handle.cancelled) return;
      if (!Geolocation) { watchViaWeb(); return; }
      Geolocation.watchPosition(options || {}, (position, err) => {
        ...
        // A watch cleared before the plugin answered has no caller left to
        // tell; the rejection is the plugin's, not the person's.
        if (handle.cancelled) return;
