...
// ---------------------------------------------------------------------------
// ios/App/App/capacitor.config.json is GENERATED, not tracked.
//
// `npx cap sync ios` writes it, and frontend/ios/.gitignore excludes it on
// purpose, so it is present on a machine that has run a sync and absent in
// every fresh clone, including this repository's CI checkout. Reading it at
// module scope used to throw ENOENT and take the whole suite down with it,
// which is how a file that passed 61 other suites came back red the first time
// it ran anywhere but one laptop.
//
// The checks it feeds are real and they stay: the bundled copy is what the
...
// So every assertion that needs this file is registered as an EXPLICIT SKIP
// when it is missing, carrying the reason in its own name, and the assertions
// that can be made from tracked files (capacitor.config.ts, project.pbxproj,
// codemagic.yaml, package.json) are split out so they run either way.
// ---------------------------------------------------------------------------
const GENERATED_CONFIG = path.join(REPO, ...IOS_APP, 'capacitor.config.json');
const hasGeneratedConfig = fs.existsSync(GENERATED_CONFIG);
const generatedConfig = () => JSON.parse(fs.readFileSync(GENERATED_CONFIG, 'utf8'));
const NEEDS_SYNC = 'SKIPPED, ios/App/App/capacitor.config.json is absent: it is'
  + ' generated by `npx cap sync ios` and gitignored, so it cannot exist in a'
  + ' fresh clone. Run the sync to make this check run';
