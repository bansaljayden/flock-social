...
describe('every NS*UsageDescription is one the app actually needs', () => {
  ...
  test('each purpose string names a specific use, in a sentence a user reads', () => {
    for (const key of usageKeys()) {
      ...
      // DESIGN-STANDARD: no em dashes in user-visible copy, and a permission prompt
...
describe('App Transport Security has no exceptions to justify', () => {
  ...
  testGenerated(g('the Cordova whitelist in the generated config.xml grants nothing'), () => {
    /* <access origin="*"/> is stock Capacitor template boilerplate and is not
       ...
       THIS USED TO ASSERT A COMMENT SAYING SO, and that was a trap. config.xml
       is generated by `npx cap sync ios` AND gitignored, so a hand-added
       explanation inside it survives exactly until the next sync and then this
       test fails for whoever ran one. It caught me on 2026-09-05 installing
       @capacitor/keyboard, which is a completely ordinary reason to sync.

       So the substance is asserted instead of the annotation. The tag is inert
       because the whitelist plugin that would act on it is a Cordova plugin
       and is not installed, and because nothing grants navigation or intent
       beyond it. Both of those are checked against tracked files, which a sync
       cannot rewrite. */
    const xml = generatedXml();
    expect(xml).toMatch(/<access origin="\*" \/>/);
    const deps = Object.keys(pkg.dependencies);
    expect(deps.filter((d) => /^cordova-plugin-whitelist$/.test(d))).toEqual([]);
    expect(xml).not.toMatch(/<allow-navigation/);
    expect(xml).not.toMatch(/<allow-intent/);
  });
});
...
const PLUGIN_CLASSES = {
  ...
  /* Keyboard joined 2026-09-05 with the composer dock. It is the only
     ...
     Swift class, and the name below is that macro's first argument. Every
     other entry here is a Swift class name, so do not "correct" this one to
     match the others' spelling by guessing.

     The JS side reaches it through hooks/useKeyboardComposer.js, which is
     what makes the chat composer ride the keyboard instead of being left
     behind by the WebView's own resize. Without the plugin that hook still
     works, on visualViewport, and the difference the plugin buys is the
     keyboardWillShow event, which arrives BEFORE the motion and is the whole
     gap between a one-step jump and a matched curve. */
  '@capacitor/keyboard': 'KeyboardPlugin',
...
describe('bundle identity and versioning agree across every file that states them', () => {
  ...
  testGenerated(g('the version in the generated config.xml is inert and does not pretend otherwise'), () => {
    /* Cordova would use this attribute; Capacitor does not, so the 1.0.0 here
       is template default rather than a version anybody ships.

       Same correction as the whitelist check above: this asserted a hand-added
       "NOT the app version" comment inside a file that `npx cap sync ios`
       regenerates from scratch and that gitignore excludes, so the annotation
       could not survive a sync and the test could not survive one either.

       What actually makes the attribute safe is that the real version lives
       somewhere else and is read from there, so that is what is asserted: the
       generated file still says 1.0.0, and the tracked project file carries a
       MARKETING_VERSION of its own that disagrees with it. If those two ever
       agree by accident, this still passes, which is correct: the danger was
       never the specific numbers, it was somebody editing config.xml believing
       it would change the shipped version. */
    const xml = generatedXml();
    expect(xml).toMatch(/version="1\.0\.0"/);
    expect(pbxproj).toMatch(/MARKETING_VERSION = /);
  });
