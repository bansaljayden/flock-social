 ...
 * copyright line. DESIGN-STANDARD A5 requires the real legal links on the public
  ...
  test.each(PAGES)('%s: the four legal links and the one mailbox', (name) => {
    ...
    for (const href of LEGAL_HREFS) {
      if (!hrefs.includes(href)) {
        throw new Error(`${name}: footer is missing the ${href} link (DESIGN-STANDARD A5)`);
      }
    }
...
describe('SiteFooter.js source hygiene', () => {
  ...
  test('no em dash outside comments (DESIGN-STANDARD A2)', () => {
