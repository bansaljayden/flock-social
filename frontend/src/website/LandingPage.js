  ...
  return (
    <div className="lp-preview-shell">
      <div className="lp-chrome">
        <div className="lp-chrome-dot lp-chrome-dot--r" />
        <div className="lp-chrome-dot lp-chrome-dot--y" />
        <div className="lp-chrome-dot lp-chrome-dot--g" />
        <span className="lp-chrome-title">Flock — Friday Night Crew</span>
      </div>
      <div className="lp-preview-panels">
        {/* LEFT PANEL */}
        <div className="lp-panel-left">
          <div className="lp-pl-header">
            <div className="lp-pl-title">Friday Night Crew</div>
            <div className="lp-pl-sub">4 going · Venue vote open</div>
          </div>
          <div className="lp-venue-cards">
            <VenueCard name="Taphouse 42" cat="Craft Beer · $$" rating="4.8" dist="0.3 mi" votes="3 votes" selected
              gradient="linear-gradient(135deg, #7c5cfc, #f97316)" label="T" />
            <VenueCard name="Liberty Bowl" cat="Bowling · $$" rating="4.6" dist="0.8 mi" votes="1 vote"
              gradient="linear-gradient(135deg, #0ea5e9, #06b6d4)" label="L" />
            <VenueCard name="Sage & Fire" cat="American · $$$" rating="4.9" dist="1.2 mi" votes="0 votes"
              gradient="linear-gradient(135deg, #f97316, #ec4899)" label="S" />
          </div>
          <div className="lp-budget-bar">
            <div className="lp-budget-row">
              <span className="lp-budget-label">Group budget</span>
              <span className="lp-budget-amount">$35/person</span>
            </div>
            <div className="lp-budget-sub">4/4 submitted · Anonymous</div>
          </div>
        </div>
        {/* RIGHT PANEL */}
        <div className="lp-panel-right">
          <div className="lp-pr-header">
            <div className="lp-avatar-stack">
              <div className="lp-avatar" style={{ background: '#7c5cfc' }}>J</div>
              <div className="lp-avatar" style={{ background: '#f97316' }}>M</div>
              <div className="lp-avatar" style={{ background: '#06b6d4' }}>A</div>
              <div className="lp-avatar" style={{ background: '#ec4899' }}>K</div>
            </div>
            <div>
              <div className="lp-pr-members">4 members</div>
              <div className="lp-pr-online">2 online</div>
            </div>
          </div>
          <div className="lp-chat-area">
            <ChatBubble side="left" avatar="M" color="#f97316" text="anyone down for bowling?" time="9:14 PM" />
            <ChatBubble side="right" text="I'm in! budget around $30?" time="9:15 PM" />
            <SystemBubble text="Budget matched for everyone — $35/person" />
            <ChatBubble side="left" avatar="A" color="#06b6d4" text="Taphouse is winning" time="9:16 PM" />
            <TypingBubble avatar="K" color="#ec4899" />
          </div>
          <div className="lp-chat-input">
            <div className="lp-chat-input-field">Message Friday Night Crew...</div>
            <div className="lp-chat-send"><IcoArrow size={14} /></div>
          </div>
        </div>
      </div>
    </div>
...
function StepCard0() {
  return (
    <>
      <div className="lp-step-input">Friday Night Crew</div>
      <div><span className="lp-step-chip lp-step-chip--active">This Friday</span><span className="lp-step-chip">Tomorrow</span></div>
      <div className="lp-step-toggle"><span className="lp-step-toggle-label">Budget matching</span><div className="lp-step-toggle-switch" /></div>
    </>
  );
}
...
function StepCard1() {
  const friends = [
    { name: 'the maintainer', init: 'J', color: '#7c5cfc', status: 'Going', cls: 'going' },
    { name: 'Marcus', init: 'M', color: '#f97316', status: 'Going', cls: 'going' },
    { name: 'Aisha', init: 'A', color: '#06b6d4', status: 'Maybe...', cls: 'maybe' },
    { name: 'Kim', init: 'K', color: '#ec4899', status: 'Invited', cls: 'invited' },
  ];
  return (
    <div className="lp-step-friends">
      {friends.map((f) => (
        <div key={f.name} className="lp-step-friend">
          <div className="lp-avatar" style={{ background: f.color, marginLeft: 0 }}>{f.init}</div>
          <span className="lp-step-friend-name">{f.name}</span>
          <span className={`lp-step-friend-status lp-step-friend-status--${f.cls}`}>{f.status}</span>
        </div>
      ))}
    </div>
  );
...
function StepCard2() {
  return (
    <>
      <div className="lp-step-budgets">
        <div className="lp-step-budget-btn">$20</div>
        <div className="lp-step-budget-btn lp-step-budget-btn--selected">$40</div>
        <div className="lp-step-budget-btn">$60</div>
        <div className="lp-step-budget-btn">$80+</div>
      </div>
      <div className="lp-step-privacy"><IcoShield size={14} /> Anonymous · Only you see this</div>
    </>
  );
}
...
function StepCard3() {
  return (
    <div className="lp-step-mini-venues">
      <VenueCard name="Taphouse 42" cat="Craft Beer" rating="4.8" dist="" votes="3" selected gradient="linear-gradient(135deg,#7c5cfc,#f97316)" label="T" />
      <VenueCard name="Liberty Bowl" cat="Bowling" rating="4.6" dist="" votes="1" gradient="linear-gradient(135deg,#0ea5e9,#06b6d4)" label="L" />
      <VenueCard name="Sage & Fire" cat="American" rating="4.9" dist="" votes="0" gradient="linear-gradient(135deg,#f97316,#ec4899)" label="S" />
    </div>
  );
}
...
function StepCard4() {
  return (
    <div className="lp-step-confirmed">
      <div className="lp-step-check-circle">
        <svg width="28" height="28" viewBox="0 0 28 28" fill="none" stroke="#4ade80" strokeWidth="3" strokeLinecap="round"><path d="M7 14.5l5 5L21 9" /></svg>
      </div>
      <div className="lp-step-confirmed-title">Flock confirmed</div>
      <div className="lp-step-confirmed-sub">Taphouse 42 · Friday 8PM · 4 attending</div>
      <div className="lp-step-confirmed-avatars">
        {['#7c5cfc','#f97316','#06b6d4','#ec4899'].map((c,i) => (
          <div key={i} className="lp-avatar" style={{ background: c, marginLeft: i ? -8 : 0 }}>{['J','M','A','K'][i]}</div>
        ))}
      </div>
    </div>
  );
...
const STEP_CARDS = [StepCard0, StepCard1, StepCard2, StepCard3, StepCard4];
...
function BudgetDemo() {
  ...
  const reset = () => { setSelected(null); setSubmitted(false); setShowResult(false); };

  if (submitted) {
    return (
      <div className="lp-budget-demo">
        <div className="lp-demo-result">
          <div className={`lp-demo-check-wrap${showResult ? ' lp-demo-check-drawn' : ''}`}>
            <svg width="28" height="28" viewBox="0 0 28 28" fill="none" stroke="#7c5cfc" strokeWidth="3" strokeLinecap="round">
              <path className="lp-demo-check-svg" d="M7 14.5l5 5L21 9" />
            </svg>
          </div>
          <div className="lp-demo-result-title">Budget Submitted</div>
          <div className="lp-demo-result-sub">Your budget: {selected}</div>
          <div className="lp-demo-progress">
            <div className={`lp-demo-progress-fill${showResult ? ' lp-demo-progress-fill--active' : ''}`} />
          </div>
          <div className="lp-demo-ceiling">Group ceiling: <strong>$48/person</strong></div>
          <button className="lp-demo-reset" onClick={reset}>Try again</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="lp-budget-demo">
      <div className="lp-demo-title">What's your budget tonight?</div>
      <div className="lp-demo-sub">This is anonymous. No one sees your answer.</div>
      <div className="lp-demo-grid">
        {['$20', '$40', '$60', '$80+'].map((v) => (
          <button key={v}
            className={`lp-demo-preset${selected === v ? ' lp-demo-preset--selected' : ''}`}
            onClick={() => setSelected(v)}>{v}</button>
        ))}
      </div>
      <div className="lp-demo-privacy"><IcoShield size={14} /> Only you can see this amount</div>
      <button className="lp-demo-submit" disabled={!selected} onClick={submit}>Submit anonymously</button>
    </div>
  );
}
