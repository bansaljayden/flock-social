...
Connect, not in the repo, and are the maintainer's to fill (TASKS.md A5, including
...
Written 2026-08-14 against the shipping build and DESIGN-STANDARD.md. Rules this
...
**22 characters.** This IS the live name: the maintainer set it in App Store Connect
...
of 2026-08-14 and are real, populated pages (DESIGN-STANDARD H7/H11).
...
### 6.9 Category (the maintainer's call, recommendation recorded)
...
is the consistent choice. Lifestyle is defensible as primary if the maintainer wants
...
| Claim | Evidence |
|---|---|
| Flocks, invites, RSVPs, votes | `backend/routes/flocks.js`, `venue_votes` table, working end-to-end list in `project documentation` |
...
| Budget privacy, ceiling after 3 submissions | Budget matching design decision in `project documentation` (client only ever receives `{ceiling, submissionCount, isReady, skipCount}`; ceiling withheld until 3 non-skipped submissions) |
...
- **Address-book contact sync is NOT collection, and on iOS it cannot even
  ...
  Contacts tab is a dead end on iOS (DESIGN-STANDARD H5 / SUBMIT-CHECKLIST P1)
...
## 9. What only the maintainer can do (updated for sections 6 to 8)
...
6. **Verify `support@flockcorp.com` exists in Cloudflare Email Routing**
   before filing the support URL page that displays it (DESIGN-STANDARD section
